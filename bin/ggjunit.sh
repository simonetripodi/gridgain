...
"${JAVA_HOME}/bin/java" ${JVM_OPTS} ${JMX_MON} ${QUIET} -DGRIDGAIN_SCRIPT -DGRIDGAIN_UPDATE_NOTIFIER=false -DGRIDGAIN_HOME="${GRIDGAIN_HOME}" -DGRIDGAIN_PROG_NAME="$0" -cp "${CP}" org.gridgain.grid.loaders.cmdline.GridCommandLineLoader "${CONFIG}"
